...
def Final_Mission():
    ...
    def Final():
        xlsxfilename='Data_Final.xlsx'
        # 1_1
        df['data_sheet1']=pd.read_excel(xlsxfilename, sheet_name='Sheet1')
        
        # 1_2
        var1=min(df['data_sheet1'].values)[0]
        str1=str(bin(var1)[2:].zfill(3))
        
        # 2_1
        df['data_sheet2']=pd.read_excel(xlsxfilename, sheet_name='Sheet2')
        
        # 2_2
        var2=df['data_sheet2'].mode()
        str2=str(var2.values[0][0])
        
        # 3_1
        df['data_sheet3']=pd.read_excel(xlsxfilename, sheet_name='Sheet3')
        
        # 3_2
        df['data_sheet3'][df['data_sheet3']<5]=1
        df['data_sheet3'][df['data_sheet3']>=5]=0 
        
        # Transform it to a long format
        df_plot=df['data_sheet3'].unstack().reset_index()
        ...
        #번호 입력받는 GUI설계
        input_Fianl = tk.Entry(Final_Mission)
        input_Fianl.pack()
        def final_output():
            input_data=input_Fianl.get()
            str3=str(input_data)        
        # 4
            str4=str1+str2+str3
            label_final_result = tk.Label(Final_Mission, text="최종결과: "+str4)
            label_final_result.pack()
            
        button1_ifelse = tk.Button(Final_Mission, text="3차원 그래프에서 보이는 내용을 입력하시오.", command=final_output)
        button1_ifelse.pack()
